...
def solve_recurrence(desc, formula):
    print(desc)
    print("Provided formula)", formula)
    c1, c0, a0, a1 = extract_parameters(formula)
    recurrence = "a(n) = " + str(c1) + "*a(n-1) + " + str(c0) + "*a(n-2)"
    print("The reconstructed recurrence is", recurrence)
    r1, r2 = solve_characteristic_equation(c0, c1)
    distinct = r1 != r2
    x0, x1 = get_coefficients(a0, a1, distinct, r1, r2)
    r1, r2, x0, x1 = round_numbers(r1, r2, x0, x1)
    formula = f"a(n) = {x0} x {r1} ^n + " + (f"{x1} x {r2}^n" if distinct else f"{x1}x n x {r2}^n")
    ...
    print("a(2) using formula", a2_formula)
    print()
...
def get_coefficients(a0, a1, distinct, r1, r2):
